...
int getbtn(int fd) {
    ...
    if (read(fd, &t, sizeof(t)) == sizeof(t)) {
        if (t.type == EV_KEY && t.value == 0)  // key released
            btn = t.code;
        else if (t.type == EV_REL && t.code == REL_WHEEL && t.value == 1)
            // vscroll up
            btn = 9001;
        else if (t.type == EV_REL && t.code == REL_WHEEL && t.value == -1)
            // vscroll down
            btn = 9002;
        else if (t.type == EV_REL && t.code == REL_HWHEEL && t.value == -1)
            // hscroll left
            btn = 9003;
        else if (t.type == EV_REL && t.code == REL_HWHEEL && t.value == 1)
            // hscroll right
...
void process_key(action_type action) {
    /*  when faking a touch event and not just faking a simple mouse click the
        orientation is ignored, and there is NOT a x,y point that turns/reverse
        the page for both portrait and landscape we could check the orientation
        via 'lipc-get-prop com.lab126.winmgr orientation'
    */
    ...
    // keep awake. at least faking touch with xdotool, we needed this. otherwise
    // the kindle sleeps due to no touch - we probably need it here too.
...
int main(int argc, char* argv[]) {
    ...
    printf("KindleLazy v0.9.1 - cearp\n\n");
    ...
    // argc is the int that stores how many arguments. always 1 or more?
    // 1 because the filename is the first argument (argv[0])
    if (argc >= 2) {
    ...
    /*  get screen res so we know where to touch
        ...
        If in book view, in portrait/landscape, X will report the correct
        respective resolution. Although 9% of the time KindleLazy is loaded in
        portrait (via KUAL) - if run from ssh whilst in landscape book view,
        the res will be landscape and be a problem. When we write a touch to
        /dev/input/event1 (touch screen), X's rotation/book rotation does not
        matter, coordinates are ALWAYS in 'portrait'. So, just in case
        KindleLazy starts up in landscape, we make sure that height < width, if
        not, swap them.
    */
    ...
    while (1) {
        if (poll(&mypoll2, 1, 10) > 0) {
            ...
            /*  I'm not sure how inefficient it is to cycle through all
                configured keys to see if the current input matches but we are
                dealing with a small amount of data so it shouldn't matter here
                :)
            */
